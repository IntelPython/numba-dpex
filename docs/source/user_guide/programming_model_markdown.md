...
Once the device becomes available the runtime selects a task whose dependencies are met for execution. The computational graph as well as the device targeted by its tasks are stored in a [SYCL queue][queue] object. The task submission is therefore always
...
We refer an interested reader to an excellent and freely available "[Data Parallel C++][dpcpp-book]" book for
...
Unified Shared Memory ([USM][sycl2020-usm]) feature. It also implements USM-based ND-array
object `dpctl.tensor.usm_ndarray` that conforms [array-API standard][array-api].
...
Automatic deduction of the execution queue from allocation queues is consitent with "[local control for data allocation target][array-api-device-support]"
in the array API standard. User has full control over memory allocation through three keyword arguments present in all [array creation functions][array-api-creation-funcs].
...
In all cases, an allocation queue object will be constructed as described [earlier](#specifying-offload-target) and
...
implicit copies and the output array is allocated on the same device as the input.
<!-- {{% /alert %}} -->

Compute follows data is the rule prescribing deduction of the execution and the allocation queue as well as the USM type for the result
...
<!-- {{% alert title="Advisory" color="warning" %}} -->
...
[array-api-device-support]: https://data-apis.org/array-api/latest/design_topics/device_support.html
[array-api-creation-funcs]: https://data-apis.org/array-api/latest/API_specification/creation_functions.html
[array-api-device-syntax]: https://data-apis.org/array-api/latest/design_topics/device_support.html#syntax-for-device-assignment
