...
Device-only functions are the functions that can be only invoked from a kernel and not from a host function.

**Data Parallel Extension for Numba** provides a special decorator
``numba_dpex.func`` used for declaration of a device function.
...
Unlike a kernel function, a device function can return values like normal
functions.
