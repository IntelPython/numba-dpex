...
try:
    ...
    # DPNP array is wrapper for dpctl usm_array.
    class DPNPArrayType(USMNdArrayType):
        """
        Numba type for ``dpnp.dpnp_array.dpnp_array``.
        """

        pass

    # This tells Numba to use the DPPYArray data layout for object of type USMNdArrayType.
    register_model(DPNPArrayType)(DPPYArrayModel)
    dppy_target.spirv_data_model_manager.register(DPNPArrayType, DPPYArrayModel)

    @typeof_impl.register(dpnp_array)
    def typeof_dpnp_array(val, c):
        """
        This function creates the Numba type (USMNdArrayType) when a dpnp. is passed.
        """
        try:
            dtype = numpy_support.from_dtype(val.dtype)
        except NotImplementedError:
        ...
        layout = "C"
        readonly = False
        return DPNPArrayType(
            dtype,
            val.ndim,
            layout,
            val._array_obj.usm_type,
            readonly=readonly,
            addrspace=address_space.GLOBAL,
        )
