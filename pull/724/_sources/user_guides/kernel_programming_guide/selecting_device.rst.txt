...
**Users are not allowed to pass mixed type of arrays to a @numba_dpex.kernel.** For example, if the first array argument to a @numba_dpex.kernel is of type :code:`numpy.ndarray`, the rest of the array argument will also have to be of type :code:`numpy.ndarray`.
...
- Array with :code:`__sycl_usm_array_interface__` attribute
     In this case the Compute Follows Data semantics is supported. Compute Follows Data stipulates that computation must be off-loaded to device where data is resident.
